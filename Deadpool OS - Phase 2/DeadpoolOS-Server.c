...
int main()
{
    ...
    char welcomeMessage[] = "***Server Version of the OS***\nWelcome to the Beta version of Deadpool OS. This is the second version of the system, more updates will follow\n\nfor help use \"help\"\n";
    char t[10];
    ...
    while (1)
    {
        ...
        // int *buffer = (int *)mmap(NULL, sizeof(int) * BUFFER_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
        ...
        send(clientSocket, osLogo, strlen(osLogo), 0);
        recv(clientSocket, t, sizeof(t), 0);
        ...
        while (1)
        {
            ...
            else if (pid == 0)
            {
                ...
                recv(clientSocket, t, sizeof(t), 0);
                send(clientSocket, OS, strlen(OS), 0);
            ...
            else
            {
                ...
                else
                {
                    ...
                    send(clientSocket, bufferFile, strlen(bufferFile) + 1, 0);
                    recv(clientSocket, t, strlen(t), 0);
                    close(fd2[0]);
                }
            }
            close(fd[0]);
